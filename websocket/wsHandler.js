...
function setupWebSocketServer(wss) {
    wss.on('connection', ws => {
        ...
        ws.uuid = null; ws.username = null; ws.sessionId = null; ws.serverId = null;
    ...
    switch (msg.type) {
        case 'AUTH': {
            try {
                ...
                ws.sessionId = msg.sessionId || 'default'; ws.serverId = msg.serverId || 'unknown';
        ...
        case 'LOCATION_UPDATE': {
            ...
            broadcastPosition(ws.uuid, ws.username, msg.sessionId || ws.sessionId, { x: msg.x, y: msg.y, z: msg.z, dimension: msg.dimension });
            break;
        }
        case 'PING_LOCATION': {
            ...
            getPingRecipients(ws.uuid, user?.ping_target || 'both', sid).forEach(uuid => { const c = connections.get(uuid); if (c) send(c, payload); });
            break;
        }
        case 'GROUP_CHAT': {
            ...
            db.prepare('SELECT uuid FROM group_members WHERE group_id=?').all(m.group_id).forEach(r => { const c = connections.get(r.uuid); if (c && c !== ws) send(c, payload); });
            break;
        }
        case 'GROUP_ANNOUNCEMENT': {
            ...
            db.prepare('SELECT uuid FROM group_members WHERE group_id=?').all(m.group_id).forEach(r => { const c = connections.get(r.uuid); if (c) send(c, payload); });
            break;
        }
        case 'BROADCAST': {
            const sid = msg.sessionId || ws.sessionId;
            ...
            getPingRecipients(ws.uuid, bc.target, sid).forEach(uuid => { const c = connections.get(uuid); if (c) send(c, payload); });
            break;
        }
        case 'STEALTH_UPDATE':
            notifyTrackers(ws.uuid, { type: 'STEALTH_NOTIFY', uuid: ws.uuid, username: ws.username, active: msg.active, until: msg.until });
            break;
        case 'NOTIFY_LOCATION_SHARE': { const c = connections.get(msg.toUuid); if (c) send(c, { type: 'LOCATION_SHARE_INCOMING', shareId: msg.shareId, fromUuid: ws.uuid, fromUsername: ws.username, durationSeconds: msg.durationSeconds }); break; }
        case 'NOTIFY_LOCATION_REQUEST': { const c = connections.get(msg.toUuid); if (c) send(c, { type: 'LOCATION_REQUEST_INCOMING', requestId: msg.requestId, fromUuid: ws.uuid, fromUsername: ws.username }); break; }
        case 'NOTIFY_FRIEND_REQUEST': { const c = connections.get(msg.toUuid); if (c) send(c, { type: 'FRIEND_REQUEST_INCOMING', requestId: msg.requestId, fromUuid: ws.uuid, fromUsername: ws.username }); break; }
        case 'NOTIFY_FRIEND_RESPONSE': { const c = connections.get(msg.fromUuid); if (c) send(c, { type: 'FRIEND_REQUEST_RESPONDED', requestId: msg.requestId, byUuid: ws.uuid, byUsername: ws.username, accepted: msg.accepted }); break; }
        case 'GROUP_EVENT': {
            const m = db.prepare('SELECT group_id FROM group_members WHERE uuid=?').get(ws.uuid);
            if (!m) break;
            db.prepare('SELECT uuid FROM group_members WHERE group_id=?').all(m.group_id).forEach(r => { if (r.uuid !== ws.uuid) { const c = connections.get(r.uuid); if (c) send(c, { type: 'GROUP_EVENT', event: msg.event, groupId: m.group_id, triggeredBy: ws.uuid }); } });
            break;
        }
    }
}

function broadcastPosition(senderUuid, senderUsername, sessionId, pos) {
    const viewers = new Set();
    const now = Math.floor(Date.now() / 1000);
    const m = db.prepare('SELECT group_id FROM group_members WHERE uuid=?').get(senderUuid);
    if (m) db.prepare('SELECT uuid FROM group_members WHERE group_id=? AND uuid!=? AND is_blind=0').all(m.group_id, senderUuid)
        .forEach(r => { const c = connections.get(r.uuid); if (c && c.sessionId === sessionId) viewers.add(r.uuid); });
    db.prepare('SELECT to_uuid FROM location_shares WHERE from_uuid=? AND expires_at>?').all(senderUuid, now)
        .forEach(s => { const c = connections.get(s.to_uuid); if (c && c.sessionId === sessionId) viewers.add(s.to_uuid); });
    db.prepare('SELECT CASE WHEN uuid1=? THEN uuid2 ELSE uuid1 END as uuid FROM friends WHERE uuid1=? OR uuid2=?').all(senderUuid, senderUuid, senderUuid)
        .forEach(f => { const c = connections.get(f.uuid); if (c && c.sessionId === sessionId) viewers.add(f.uuid); });
...
function getPingRecipients(senderUuid, target, sessionId) {
    const out = new Set();
    ...
    if (target === 'group' || target === 'both') {
        ...
        if (m) db.prepare('SELECT uuid FROM group_members WHERE group_id=? AND uuid!=?').all(m.group_id, senderUuid)
            .forEach(r => { const c = connections.get(r.uuid); if (c && c.sessionId === sessionId && !ignored.has(r.uuid)) out.add(r.uuid); });
    }
    if (target === 'friends' || target === 'both') {
        db.prepare('SELECT CASE WHEN uuid1=? THEN uuid2 ELSE uuid1 END as uuid FROM friends WHERE uuid1=? OR uuid2=?').all(senderUuid, senderUuid, senderUuid)
            .forEach(f => { const c = connections.get(f.uuid); if (c && c.sessionId === sessionId && !ignored.has(f.uuid)) out.add(f.uuid); });
    }
    return out;
}
