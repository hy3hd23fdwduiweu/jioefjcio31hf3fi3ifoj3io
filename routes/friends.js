...
router.get('/', (req, res) => {
    ...
    const friends = db.prepare(`
        SELECT CASE WHEN uuid1=? THEN uuid2 ELSE uuid1 END as friend_uuid,
               CASE WHEN uuid1=? THEN color_for_1 ELSE color_for_2 END as my_color,
               u.username
        FROM friends f JOIN users u ON u.uuid=(CASE WHEN uuid1=? THEN uuid2 ELSE uuid1 END)
        WHERE uuid1=? OR uuid2=?
    `).all(uuid, uuid, uuid, uuid, uuid);
...
router.post('/request', (req, res) => {
    ...
    if (!db.prepare('SELECT uuid FROM users WHERE uuid=?').get(targetUuid))
        return res.status(404).json({ error: 'Player not found' });
    if (db.prepare('SELECT * FROM ignores WHERE uuid=? AND ignored_uuid=?').get(targetUuid, uuid))
        return res.status(403).json({ error: 'Player is not accepting requests from you' });
    if (db.prepare('SELECT * FROM friends WHERE (uuid1=? AND uuid2=?) OR (uuid1=? AND uuid2=?)').get(uuid, targetUuid, targetUuid, uuid))
        return res.status(409).json({ error: 'Already friends' });
    if (db.prepare('SELECT COUNT(*) as c FROM friends WHERE uuid1=? OR uuid2=?').get(uuid, uuid).c >= MAX_FRIENDS)
        return res.status(400).json({ error: `Max ${MAX_FRIENDS} friends` });
    if (db.prepare("SELECT * FROM friend_requests WHERE from_uuid=? AND to_uuid=? AND status='pending'").get(uuid, targetUuid))
...
router.get('/requests', (req, res) => {
    const requests = db.prepare(`
        SELECT fr.id, fr.from_uuid, u.username, fr.created_at
        FROM friend_requests fr JOIN users u ON u.uuid=fr.from_uuid
        WHERE fr.to_uuid=? AND fr.status='pending' ORDER BY fr.created_at DESC
    `).all(req.player.uuid);
...
router.post('/color', (req, res) => {
    const { uuid } = req.player;
    const { friendUuid, color } = req.body;
